...
def train_cont_cities_er(data_root, save_name, loss_type='CE', seed=0, multihead=False):
    torch.manual_seed(seed)
    np.random.seed(seed)
    random.seed(seed)
...
def train_cont_cities_covarnull(data_root, save_name, loss_type='CE', seed=0, multihead=False):
    torch.manual_seed(seed)
    np.random.seed(seed)
    random.seed(seed)
    ...
    vgg_model = VGGNet(model='vgg19', requires_grad=True, bias=False)
    if multihead:
        model = FCN8sMHCovarNull(lr=lr, pretrained_net=vgg_model, n_class=34, n_tasks=task_num).to(device)
    else:
        model = FCN8s(pretrained_net=vgg_model, n_class=34).to(device)

    #print model param name 
    for name, param in model.named_parameters():
    ...
    for t_cnt in range(task_num):
        ...
        model.reset_optim(lr=lr, t_id=t_cnt)
        ...
        for epoch in range(nepoch):
            for i, data in enumerate(dl_train):
                ...
                loss = criterion(output, mask)
                loss.backward()
                model.model_optimizer.step(t_id = t_cnt)
        ...
        with torch.no_grad():
            dl_train = DataLoader(train_datasets[t_cnt], batch_size=bs, shuffle=True)
            model.update_optim_transforms(dl_train)
...
def train_cont_cities_ewc_weighted(data_root, save_name, loss_type='CE', seed=0, multihead=False):
    torch.manual_seed(seed)
    np.random.seed(seed)
    random.seed(seed)
    ...
    for t_cnt in range(task_num):
        ...
        register_ewc_params(model, dl_train, loss_type='focal_weighted', multihead=multihead, t_cnt=t_cnt)
...
# train_cont_cities_ewc('leftImg8bit', 'clean_all_cities_VGG19_CEloss_ewc_t6_1pt_multihead_w1e12', 
#                       loss_type='CE', multihead=True)

# train_cont_cities_ewc_weighted('leftImg8bit', 'clean_all_cities_VGG19_CEloss_ewcWeighted_t6_1pt_multihead_w1e12', 
...
# train_cont_cities_covarnull('leftImg8bit', 
#                             'clean_all_cities_VGG19_focalloss_CovarNull_t6_1pt_multihead', 
#                             loss_type='focal', multihead=True)
